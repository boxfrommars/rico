...
контроллер отвечающий за аутентификацию -- `\Rico\Auth\UsersController`, в него частично (!) перенесены методы контроллера из пакета Confide (только касающиеся входа/выхода),
