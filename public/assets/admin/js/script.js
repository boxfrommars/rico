    ...
    if (!$form.disabledWhileSending) {
        ...
        $.ajax({
            ...
            success: function (data, textStatus, jqXHR) {
                $form.disabledWhileSending = true;
    ...
    $(this).fileupload({
        ...
        formData: [{
            ...
            if (result.success === false) {
                ...
                $container.find('img').attr('src', result.path);
                $container.find('.fancybox').attr('href', result.path);
                $container.find('.image-input').val(result.filename);
                App.notify.success('Изображение загружено');
            }
        }
    })
});
    ...
    $(this).fileupload({
        ...
        formData: [{
            ...
            if (result.success === false) {
                ...
                var $container = $(this).parents('.upload-container');
                $container.find('.upload-result').text(result.filename);
                $container.find('.upload-result').attr('href', result.path);
                $container.find('.file-input').val(result.filename);
                App.notify.success('Файл загружен');
            }
        }
    })
});

$('.js-upload-image-remove').on('click', function(e){
    var $container = $(this).parents('.upload-container');
    $container.find('img').attr('src', '');
    $container.find('.fancybox').attr('href', '');
    $container.find('.image-input').val('');
    e.preventDefault();
});

$('.js-upload-file-remove').on('click', function(e){
    var $container = $(this).parents('.upload-container');
    $container.find('.upload-result').text('');
